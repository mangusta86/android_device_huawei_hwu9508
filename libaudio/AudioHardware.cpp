...
//#define LOG_NDEBUG 0
#define LOG_TAG "AudioHardwareHI6421"
#include <utils/Log.h>
        ...
        uint32_t devices, int *format, uint32_t *channels, uint32_t *sampleRate, status_t *status)
{
    { // scope for the lock
        ...
        AudioStreamOutHI6421* out = new AudioStreamOutHI6421();
        ...
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    AudioStreamInHI6421* in = new AudioStreamInHI6421();
...
void AudioHardware::closeInputStream(AudioStreamIn* in) {
    ...
    ssize_t index = mInputs.indexOf((AudioStreamInHI6421 *)in);
    if (index < 0) {
...
status_t AudioHardware::doRouting()
{
    ...
    int (*HI6421_enable_audpp)(int);
    HI6421_enable_audpp = (int (*)(int))::dlsym(acoustic, "HI6421_enable_audpp");
    int audProcess = (ADRC_DISABLE | EQ_DISABLE | RX_IIR_DISABLE);
    AudioStreamInHI6421 *input = getActiveInput_l();
    ...
    if (sndDevice != -1 && sndDevice != mCurSndDevice) {
        ...
        if ((*HI6421_enable_audpp) == 0 ) {
            ALOGE("Could not open HI6421_enable_audpp()");
        } else {
            HI6421_enable_audpp(audProcess);
        }
...
AudioHardware::AudioStreamInHI6421 *AudioHardware::getActiveInput_l()
{
    for (size_t i = 0; i < mInputs.size(); i++) {
        // return first input found not being in standby mode
        // as only one input can be in this state
        if (mInputs[i]->state() > AudioStreamInHI6421::AUDIO_INPUT_CLOSED) {
            return mInputs[i];
        }
    }
...
AudioHardware::AudioStreamOutHI6421::AudioStreamOutHI6421() :
    mHardware(0), mFd(-1), mStartCount(0), mRetryCount(0), mStandby(true), mDevices(0)
{
}

status_t AudioHardware::AudioStreamOutHI6421::set(
...
ssize_t AudioHardware::AudioStreamOutHI6421::write(const void* buffer, size_t bytes)
{
    // ALOGD("AudioStreamOutHI6421::write(%p, %u)", buffer, bytes);
...
status_t AudioHardware::AudioStreamOutHI6421::standby()
{
...
status_t AudioHardware::AudioStreamOutHI6421::dump(int fd, const Vector<String16>& args)
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
    result.append("AudioStreamOutHI6421::dump\n");
...
bool AudioHardware::AudioStreamOutHI6421::checkStandby()
{
    return mStandby;
}


status_t AudioHardware::AudioStreamOutHI6421::setParameters(const String8& keyValuePairs)
{
    AudioParameter param = AudioParameter(keyValuePairs);
    String8 key = String8(AudioParameter::keyRouting);
    status_t status = NO_ERROR;
    int device;
    ALOGV("AudioStreamOutHI6421::setParameters() %s", keyValuePairs.string());
...
String8 AudioHardware::AudioStreamOutHI6421::getParameters(const String8& keys)
{
    AudioParameter param = AudioParameter(keys);
    String8 value;
    String8 key = String8(AudioParameter::keyRouting);
    ...
    ALOGV("AudioStreamOutHI6421::getParameters() %s", param.toString().string());
    return param.toString();
}

status_t AudioHardware::AudioStreamOutHI6421::getRenderPosition(uint32_t *dspFrames)
{
...
AudioHardware::AudioStreamInHI6421::AudioStreamInHI6421() :
...
status_t AudioHardware::AudioStreamInHI6421::set(
        AudioHardware* hw, uint32_t devices, int *pFormat, uint32_t *pChannels, uint32_t *pRate,
        AudioSystem::audio_in_acoustics acoustic_flags)
{
    ...
    ALOGV("AudioStreamInHI6421::set(%d, %d, %u)", *pFormat, *pChannels, *pRate);
    if (mFd >= 0) {
     ...
     */
    int (*HI6421_set_audpre_params)(int, int);
    HI6421_set_audpre_params = (int (*)(int, int))::dlsym(acoustic, "HI6421_set_audpre_params");
    status = HI6421_set_audpre_params(audpre_index, tx_iir_index);
    if (status < 0)
        ALOGE("Cannot set audpre parameters");

    int (*HI6421_enable_audpre)(int, int, int);
    HI6421_enable_audpre = (int (*)(int, int, int))::dlsym(acoustic, "HI6421_enable_audpre");
    mAcoustics = acoustic_flags;
    status = HI6421_enable_audpre((int)acoustic_flags, audpre_index, tx_iir_index);
    if (status < 0)
...
AudioHardware::AudioStreamInHI6421::~AudioStreamInHI6421()
{
    ALOGV("AudioStreamInHI6421 destructor");
    standby();
}

ssize_t AudioHardware::AudioStreamInHI6421::read( void* buffer, ssize_t bytes)
{
    ALOGV("AudioStreamInHI6421::read(%p, %ld)", buffer, bytes);
...
status_t AudioHardware::AudioStreamInHI6421::standby()
{
...
status_t AudioHardware::AudioStreamInHI6421::dump(int fd, const Vector<String16>& args)
{
    const size_t SIZE = 256;
    char buffer[SIZE];
    String8 result;
    result.append("AudioStreamInHI6421::dump\n");
    snprintf(buffer, SIZE, "\tsample rate: %d\n", sampleRate());
    result.append(buffer);
    snprintf(buffer, SIZE, "\tbuffer size: %d\n", bufferSize());
    result.append(buffer);
...
status_t AudioHardware::AudioStreamInHI6421::setParameters(const String8& keyValuePairs)
{
    AudioParameter param = AudioParameter(keyValuePairs);
    String8 key = String8(AudioParameter::keyRouting);
    status_t status = NO_ERROR;
    int device;
    ALOGV("AudioStreamInHI6421::setParameters() %s", keyValuePairs.string());
...
String8 AudioHardware::AudioStreamInHI6421::getParameters(const String8& keys)
{
    AudioParameter param = AudioParameter(keys);
    String8 value;
    String8 key = String8(AudioParameter::keyRouting);
    ...
    ALOGV("AudioStreamInHI6421::getParameters() %s", param.toString().string());
    return param.toString();
}
